...
public class BlockBottle extends Block implements ITileEntityProvider, IModelInfo
{
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	{
		...
		{
			placer.addChatMessage(new TextComponentString(TextFormatting.GRAY + "TIP: " + TextFormatting.YELLOW + "Right click the bottle with any potion to fill it."));
			placed = true;
		}
	}
		...
		if (tileEntity instanceof TileEntityBottle)
		{
			...
			if (playerIn.isSneaking())
			{
				if (tileEntityBottle.hasPotion())
				{
					if (!worldIn.isRemote) 
					{
						playerIn.addChatMessage(new TextComponentString(TextFormatting.GRAY + tileEntityBottle.getPotionName() + " " + tileEntityBottle.getPotionLevel()));
					}
					else
					{
						if(!viewed)
						{
							playerIn.addChatMessage(new TextComponentString(TextFormatting.GRAY + "TIP: " + TextFormatting.YELLOW + "Right click with an empty glass bottle to extract the liquid."));
							viewed = true;
						}
					}
				}
			...
			if (current != null)
			{
				if (tileEntityBottle.addPotion(current))
				{
					...
					{
						playerIn.addChatMessage(new TextComponentString(TextFormatting.GRAY + "TIP: " + TextFormatting.YELLOW + "Crouch and right click the bottle with hand to see whats in it."));
						inserted = true;
					}
				}
