...
public class BlockCanvas extends BlockFurniture implements ITileEntityProvider
{
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	{
		...
		{
			placer.sendMessage(new TextComponentString(TextFormatting.GRAY + "TIP: " + TextFormatting.YELLOW + "Place a painting on the canvas to view it in style!"));
			placed = true;
		}
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	{
		if(!worldIn.isRemote)
		{
			...
			if(tileEntity instanceof TileEntityCanvas)
			{
				...
				if(canvas.hasArt)
				{
					worldIn.spawnEntity(new EntityItem(worldIn, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, new ItemStack(Items.PAINTING)));
				}
			}
		}
					...
					if(current != null)
					{
						if(current.getItem() == Items.PAINTING)
						{
							if(tileEntityCanvas.addArt())
							{
								...
								{
									playerIn.sendMessage(new TextComponentString(TextFormatting.GRAY + "TIP: " + TextFormatting.YELLOW + "Crouch and right click with hand to cycle through " + TextFormatting.YELLOW + "paintings."));
									art = true;
								}
								current.shrink(1);
							}
